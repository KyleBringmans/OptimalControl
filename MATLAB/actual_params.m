function p = actual_params(varargin)
...
p.m = 1.850;            % kg ...... total mass 1.850
p.L = 0.27;             % m ....... arm length 0.27
p.rho = 1.225;          % kg/m3 ... air density (nominal, at 15C, sea level) 1.225(robust)
...
p.Dp = 11*2.54/100;     % m ....... propeller diameter (11")

% motors
p.Kv = 700;             % rpm/V ... motor speed constant 700
...
% Moments of intertia (robust)
p.Ixx=0.0321;             % kgm2 .... Ixx moment of inertia 0.0321
p.Iyy=0.0340;             % kgm2 .... Iyy moment of inertia 0.0340
p.Izz=0.0575;             % kgm2 .... Izz moment of inertia 0.0575
...
% covarDynamics for attitude (covariances for 'v' and 'w')
% v represents measurement noise
% w represents noise introduced by an actuator
% p.covarDynamics_att = 1e-6 * eye(3);
p.covarSensors_att = diag([0.0005 0.0005 0.0005 0.00007 0.00007 0.00007]);
p.covarDynamics_att = 1e-3 * eye(3);
% p.covarSensors_att = diag([0.0069^2, 0.007^2, 0.0069^2, 0.002^2, 0.002^2, 0.002^2]);

% covarDynamics for altitude (covariances for 'v' and 'w')
% v represents measurement noise
% w represents noise introduced by an actuator
p.covarSensors_alt = 1e-2;
p.covarDynamics_alt = 1e-4;

% covarDynamics for navigation (covariances for 'v' and 'w')
% v represents measurement noise
% w represents noise introduced by an actuator
p.covarSensors_nav = 1e-3 * eye(2);
p.covarDynamics_nav = 1e-5 * eye(2);
