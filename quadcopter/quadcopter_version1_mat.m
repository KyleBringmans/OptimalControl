...
%R = eye(4,4)*0.1;
%for lqg
Q = diag([80 80 150 25 25 1 100 100 1 10^-1 10^-1 10^-2]);
R = eye(4,4)*0.1;
...
%R_i = eye(4,4)*0.001;
%with kalman
Q_i = diag([30 30 15 3 3 1 100 100 100 10^-1 10^-1 10^-2 380 380 350]);
R_i = eye(4,4)*0.001;
...
%shit symmetry....
%Qk = diag([100 100 100 200 200 100 55 55 35 10^-1 10^-1 10^-1]*10);
Qk = diag([0.5 0.5 0.001 200 200 10^-1 15 15 10 10^-1 10^-1 10^-1]*10);
%Q(6,6) = 100;
